...
THIS CODE CONTAINST THE MAIN IMPLEMENTATION FOR THE PENSIONS STATIC EXAMPLE WITH MULTIPLE COMPETITORS

The scheme follows that of the original static.py, although now we have to account for several other
competitors that must be added to the scene. 
...
class static_retirement_decision:
    ...
    def __init__(self, params):
        ...
        ########################################
        # Other banks parameters (competitors) #
        ########################################

        self.lambda2 = params["lambda2"]                        # Penalty for early leaving 
        self.T2 = params["T2"]                                  # Peramenence required 
        self.probs2 = params["probs2"]                          # Probs. of early leaving
        self.h2_values = params["h2_values"]                    # Possible returns offered 
        self.h2_probs = params["h2_probs"]                      # Probs. of each possible return 

        self.n_comp = len(self.lambda2)
    ...
    def estimate_prob_acceptance(self, h):
        ...
        count = 0           # Count of how many times the client prefers our offer to the competitors'
        ...
        for i in range(self.N):
            ...
            # The rho sample is shared between bank 1 and 2
            ...
            # Compute the utilities attained by the client for each of the competitor's banks 
            ut2 = []
            for j in range(self.n_comp):

                # Sample a possible bonus for the j_th competitor's bank, given by "h2_values",
                # with probabilities "p" (h2_probs)
                h2 = np.random.choice(self.h2_values[j], p = self.h2_probs[j])

                ut_comp_bank = self.compute_utility(
                    self.probs2[j],
                    h2,
                    self.x,
                    self.T2[j],
                    rho_sample,
                    self.money_max,
                    self.lambda2[j])  # utility for the client if he chooses other bank

                ut2.append(ut_comp_bank)

            u1.append(ut1)
            u2.append(np.max(ut2))

            # Update the count of the successful results to estimate the
            # probability (we compete against the best offer of the competitors)
            if ut1 > np.max(ut2):
                count += 1
