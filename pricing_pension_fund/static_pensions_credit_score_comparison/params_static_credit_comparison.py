...
###################################
# Bank 2 parameters (competitors) #
###################################

# All competitors here are modelled using the same parameters for illustrative purposes
# We change the number of competitors for each simulation

lambda2 = 0.8       # Percetange of penalty on the bonus for early leaving
T2 = 8              # Minimum required time for the client to stay, if the offer by the competitor is accepted
...
# Possible offers from the competitors and their respective probabilities
h2_values = np.array([0.025, 0.03, 0.035, 0.04, 0.045, 0.05, 0.055, 0.06, 0.065, 0.07])

h2_probs_low_score = np.array([0.3, 0.2, 0.15, 0.10, 0.05, 0.05, 0.05, 0.05, 0.025, 0.025])
h2_probs_high_score = np.array([0.025, 0.025, 0.05, 0.05, 0.05, 0.10, 0.15, 0.2, 0.3, 0.05])
...
simulation_params = {'x' : x, 'rho_client_low' : rho_client_low, 'rho_client_high' : rho_client_high, 
    ...
    'lambda2' : lambda2, 'T2' : T2, 'probs2' : probs2, 'h2_values' : h2_values, 
    'h2_probs_high_score' : h2_probs_high_score, 'h2_probs_low_score' : h2_probs_low_score}
